...
for(chr in 1:22){{
  snp_readBed(paste0("theor_herit/wpc_",group,"_",chr,".bed"))
}}
...
for (chr in 1:22) {{
  rds_file <- paste0("theor_herit/wpc_",group,"_",chr,".rds")
  ...
  save_run(snp_cor(G, ind.row=1:nrow(G),ind.col = ind.chr,
                   ...
                   ncores = NCORES),
           file = paste0("theor_herit/tmp-data/wpc_",group,"_corr_chr", chr, ".rds"))
...
#  POS <- ukb$map$physical.pos
 ...
 # ind.chr3 <- match(ind.chr2,sumstats$rsid[ind.chr])
  ...
  corr0 <- readRDS(paste0("theor_herit/tmp-data/wpc_",group,"_corr_chr", chr, ".rds"))#[ind.chr3, ind.chr3]
...
final_table <- tibble(phenotype=phenotype,
                      h2 = h2_est, 
			group = rep(group,3))
final_name <- paste0("theor_herit/wpc_",group,"_h2.tsv")
...
--chr $i \
--keep data/theory/wpc_{}.txt \
--make-bed \
--out theor_herit/wpc_{}_$i

rm theor_herit/wpc_{}_$i.log
rm theor_herit/wpc_{}_$i.nosex
done

Rscript transethnic_scripts/ldsc_herit_wpc_{}.R
rm transethnic_scripts/ldsc_herit_wpc_{}.R

for i in $(seq 1 22);
do
rm theor_herit/wpc_{}_$i.b*
rm theor_herit/wpc_{}_$i.f*
rm theor_herit/wpc_{}_$i.r*
rm theor_herit/tmp-data/wpc_{}_corr_chr$i.rds
done

rm transethnic_scripts/ldsc_herit_wpc_{}.sh
...
if __name__ == "__main__":
    ...
    for group in groups:
        with open(f'transethnic_scripts/ldsc_herit_wpc_{group}.R','w') as f:
            f.write(main1(group))
        with open(f'transethnic_scripts/ldsc_herit_wpc_{group}.sh', 'w') as f:
            f.write(main2(group))
        os.system(f'sbatch transethnic_scripts/ldsc_herit_wpc_{group}.sh')

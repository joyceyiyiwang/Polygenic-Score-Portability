...
ld <- data.frame()
  for (chr in 1:22){{
    file <- paste0("data/theory/wpc_",group,"_",chr,".ld")
    delim <- read_delim(file, delim = ' ', trim_ws = T, col_types = cols())
    delim <- delim %>% select(-X8) %>% mutate(R2 = R^2) %>%
      select(SNP_A,SNP_B,R,R2)
...
print(paste0("WPC LD,",group,",",rel_r2))
print(group)
print(num)
print(den)
print(rel_r2)
...
Rscript transethnic_scripts/pc_{}1.R
rm transethnic_scripts/pc_{}1.R
rm transethnic_scripts/pc_{}1.sh
"""
...
if __name__ == "__main__":
    ...
    for group in groups:
        with open(f'transethnic_scripts/pc_{group}1.R','w') as f:
            f.write(main1(group))
        with open(f'transethnic_scripts/pc_{group}1.sh', 'w') as f:
            f.write(main2(group))
        os.system(f'sbatch transethnic_scripts/pc_{group}1.sh')

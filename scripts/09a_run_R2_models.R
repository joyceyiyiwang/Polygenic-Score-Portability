...
#        threshold = str_extract_all(string = file, pattern = '[0-9]+')[[1]][1] %>% as.integer
#      ) %>%
...
combined <- function(prs_df, type, line){
  ...
  if(line ==5){
    ...
    tb <- plot_df %>%
      ...
      do(
        ...
        ls = loess(incremental_r2~mean_values, data=.)
      )  %>%
      ungroup() 
    ...
    tb <- tb %>%
      ...
      mutate(group_number = rep(1:40,10*4)) %>%
...
for (i in c(1,5)){
  ...
  comb_fst <- combined(as.data.frame(all_list[1]),"FST",i)
  ggsave(paste0('img/FINAL_COMB_FST',i,"_40.png"),comb_fst,width=20,height=10,dpi=1000)

#comb_wpc <- combined(as.data.frame(all_list[3]),as.data.frame(all_list[4]), "WPC",i)
#ggsave(paste0('img/FINAL_COMB_WPC',i,"_40.png"),comb_wpc,width=20,height=10,dpi=1000)
}

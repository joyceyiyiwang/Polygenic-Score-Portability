...
non_prs_full_df <- load_non_prs_df("final_fst_all_chrom",num_fst_groups = 100, n_buckets=100, first = 50) # For comparison
...
median_fst_full = get_median_fst(non_prs_full_df)

chrom_compare = cbind.data.frame(median_fst, median_fst_full = median_fst_full$median_fst)
r_2 = round(summary(lm(chrom_compare$median_fst_full~chrom_compare$median_fst))$r.squared, 3)
p = round(summary(lm(chrom_compare$median_fst_full~chrom_compare$median_fst))[[4]][2,4], 3)
chrom_compare %>% ggplot(aes(x = median_fst, y = median_fst_full)) + 
  geom_point() +
  geom_smooth(method="lm",se=F, color = "red") + 
  xlab("Chromosome 1") +
  ylab("All Autosomes") +
  ggtitle("Correlation between Median FST of FST Bins") +
  labs(subtitle = paste0("r-squared = ", r_2, " (p = ", p, ")"))
...
combined <- function(prs_df, type, line, trait, c){
  ...
  if (type=="FST"){
    ...
    caption1 =  "Total of 100 groups composed of 937-938 each, except for the first group (50x larger)."
    #delta <- 0.05
    beta <- "\u03b2 (p)"
  }
  ...
  plot <- plot +
    ...
    b = c()
    for(i in 1:nrow(tb)){
      r = c(r, round(summary(tb[[3]][[i]])$r.squared,3))
      b = c(b, paste0(round(summary(tb[[3]][[i]])$coef[2,c(1)],3),
                      " (",
                      round(summary(tb[[3]][[i]])$coef[2,c(4)],3),
                      ")"))
    }
    tb <- tb %>% ungroup() %>%
      mutate(r2 = r, Coef = b) %>%
  ...
  if(line ==2){
    ...
    b2 = c()
    for(i in 1:nrow(tb)){
      r1 = c(r1, round(summary(tb[[3]][[i]])$r.squared,3))
      b1 = c(b1, paste0(round(summary(tb[[3]][[i]])$coef[2,c(1)],3),
                      " (",
                      round(summary(tb[[3]][[i]])$coef[2,c(4)],3),
                      ")"))
      r2 = c(r2, round(summary(tb[[4]][[i]])$r.squared,3))
      b2 = c(b2, paste0(round(summary(tb[[4]][[i]])$coef[2,c(1)],3),
                        " (",
                        round(summary(tb[[4]][[i]])$coef[2,c(4)],3),
                        ")"))
    }
    tb <- tb %>% ungroup() %>%
...
for(i in 1:10){
  for(j in c(1, 2, 5)){
    comb_fst <- combined(as.data.frame(all_list[[1]]),"FST",j, pheno[i], colors[i])
    ggsave(paste0('img/FINAL_COMB_FST_',j, "_",pheno[i],"_100_50.png"),comb_fst,width=20,height=10,dpi=1000)
  }
}
